...
stego_method = "EECS"
bitrate = 128                   # bitrates = [128, 320]
capacity = 4                    # alpha = 1 / capacity capacity = [2, 4, 5, 10] -> alpha = [0.5, 0.25, 0.2, 0.1]
embedding_rate = 10
...
# EECS
cover_train_files_path = os.path.join(data_dir, "cover/" + str(bitrate), 'train')
cover_val_files_path = os.path.join(data_dir, "cover/" + str(bitrate), 'validation')
stego_train_files_path = os.path.join(data_dir, "stego/EECS/" + stego_dir_name, 'train')
stego_val_files_path = os.path.join(data_dir, "stego/EECS/" + stego_dir_name, 'validation')
...
ratio = 0.5                                                                     # 训练集所占比例
start_index, end_index = 0, 10000                                               # 实际用于训练和验证的数据集规模

files_list = get_files_list(cover_train_files_path, start_index, end_index)     # 文件列表(用于计算iteration)
files_num = len(files_list)                                                     # 总文件数量
iters = files_num // batch_size                                                 # 每个epoch的iters
...
for epoch in range(n_epoch):
    ...
    train_data_list, train_label_list = read_data(cover_train_files_path,
                                                  stego_train_files_path,
                                                  start_index,
                                                  end_index)  # 读取文件列表(默认shuffle)

    val_data_list, val_label_list = read_data(cover_val_files_path,
    ...
    print("epoch: %d, learning_rate: %f -- train loss: %f, train acc: %f, validation loss: %f, validation acc: %f"
          % (epoch + 1, init_learning_rate, train_loss / n_batch_train, train_acc / n_batch_train,
